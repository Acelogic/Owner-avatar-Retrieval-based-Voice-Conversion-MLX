...
def extract_small_model(path, name, sr, if_f0, version, epoch, step):
    try:
        ckpt = mx.load(path)
        pth_file = f"{name}.npz"
        pth_file_old_version_path = os.path.join("logs", f"{pth_file}_old_version.npz")
        opt = OrderedDict(
            weight={
                key: mx.array(value, dtype=mx.float16) for key, value in ckpt.items() if "enc_q" not in key
            }
        )
        if "model" in ckpt:
            ckpt = ckpt["model"]
        ...
        for key in ckpt.keys():
            if "enc_q" in key:
                continue
            opt["weight"][key] = mx.array(ckpt[key], dtype=mx.float16)
        ...
        model = mx.load(pth_file_old_version_path)
        model = replace_keys_in_dict(
        ...
        mx.save(pth_file_old_version_path, model)
